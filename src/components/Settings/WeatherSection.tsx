...
export default function WeatherSection() {
  const [isWeatherShown, setIsWeatherShown] = useState<boolean | null>(null);
  const [apiText, setApiText] = useState<string | null>(null);

  const getData = async () => {
    try {
      const request = await axios.get("/api/weather/manage");

      setIsWeatherShown(request.data.weather.show);
      setApiText(request.data.weather.token);
    } catch (error) {
  ...
  if (isWeatherShown === null || apiText === null) {
    return (
  ...
  return (
    <div className="rounded-lg bg-[#202020] p-4 mb-4">
      <h1 className="bold text-lg">Weather</h1>
      <div className="flex mb-2">
        ...
        <label className="relative inline-flex items-center cursor-pointer">
          <input
            type="checkbox"
            checked={isWeatherShown}
            onChange={(e) => setIsWeatherShown(e.target.checked)}
